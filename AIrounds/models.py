...
class InterviewSession(models.Model):
    ...
    application = models.ForeignKey('jobs.JobApplication', on_delete=models.SET_NULL, null=True, blank=True, related_name='interview_sessions')
...
class InterviewRound(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    session = models.ForeignKey(InterviewSession, on_delete=models.CASCADE, related_name='rounds')
    
    # Strategy Tier (Category)
    ...
    designation = models.CharField(
        ...
        default='TECHNICAL_SCREENING'
    )
    
    # Evaluation Depth
    difficulty = models.CharField(
        max_length=20, 
        choices=InterviewSession.EVALUATION_DEPTH_CHOICES, 
        default='MID'
    )
...
class CandidateInterviewLink(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    session = models.OneToOneField(InterviewSession, on_delete=models.CASCADE, related_name='active_link')
    token = models.UUIDField(default=uuid.uuid4, unique=True, db_index=True)

    # Exam credentials (NOT Django auth — standalone exam access)
    exam_username = models.CharField(max_length=100, unique=True, db_index=True, default='', blank=True)
    ...
    def _generate_credentials(self):
        ...
        first = candidate.first_name.lower().replace(' ', '') if candidate.first_name else 'candidate'
