...
class AIScreeningHistoryView(APIView):
    ...
    def get(self, request):
        reports = AIScreeningReport.objects.filter(recruiter=request.user, is_deleted=False).order_by('-created_at')
...
class AnalyzeResumesView(APIView, ResponseMixin):
    ...
    def post(self, request, job_id):
        ...
        if not applications:
            all_scored_qs = job.applications.filter(ai_score__isnull=False, is_deleted=False).order_by("-ai_score")
            ...
            if total_scored > 0:
                ...
                for rank, cand in enumerate(all_scored_qs[:25], 1):
                    ...
                    if not summary:
                        summary = (cand.ai_analysis[:500] + "...") if cand.ai_analysis and len(cand.ai_analysis) > 500 else (cand.ai_analysis or "No analysis available.")
                ...
                return self.build_response(
                    ...
                    {"processed_count": 0, "total_applicants": total_scored, "top_candidates": results_data}
                )
            return self.build_response("error", "No pending applications found.", {}, status.HTTP_400_BAD_REQUEST)
        ...
        report = AIScreeningReport.objects.create(
            job_id=job.id, job_title=job.title, recruiter=request.user,
            results={
                "status": "processing", "job_id": str(job.id),
                "count": len(applications), "total_applicants": len(applications), "top_candidates": []
            }
        )
        ...
        def run_screening_background(job_id, user_id, app_ids, report_id):
            ...
            try:
                ...
                for rank, cand in enumerate(all_scored[:50], 1):
                    ...
                    if not summary: summary = (cand.ai_analysis[:500] + "...") if cand.ai_analysis and len(cand.ai_analysis) > 500 else (cand.ai_analysis or "No analysis available.")
                    
                    results_data.append({
                        "id": str(cand.id), "rank": rank, "name": f"{cand.applicant.first_name} {cand.applicant.last_name}",
                        "email": cand.applicant.email, "score": cand.ai_score, "summary": summary, "analysis": analysis_obj
                    })
        ...
        thread = threading.Thread(target=run_screening_background, args=(str(job.id), request.user.id, app_ids, str(report.id)))
        thread.daemon = True
        thread.start()

        return self.build_response(
            "success", f"Screening started for {len(applications)} candidates.",
...
class DeleteScreeningReportView(APIView, ResponseMixin):
    ...
    def delete(self, request, report_id):
        try:
            report = AIScreeningReport.objects.get(id=report_id, recruiter=request.user, is_deleted=False)
            ...
            return self.build_response("error", "Report not found.", {}, status.HTTP_404_NOT_FOUND)
